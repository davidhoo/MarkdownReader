...
- **最近打开菜单在 v2.4.3 中不显示实际记录**：修复 SwiftUI Commands 在无最近项时把子菜单扁平化为单项的问题，改为通过 AppKit 的 `NSMenuDelegate` 在展开时动态填充最近文件/目录；同时把侧边栏目录树中的文件切换也纳入最近打开记录，不再只依赖显式打开入口。空状态、最近项、清除菜单与多语言标题均已补齐

### 测试

- 全部 288 个测试通过（v2.4.3 为 285 个；新增菜单动态填充、侧边栏文件加载记录、临时 Untitled 文件不记录等回归测试）
