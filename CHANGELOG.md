...
- **2.4.4 启动后卡死并持续占用 CPU/内存**：修复最近打开菜单监听自身菜单更新、反复重建菜单项形成的通知循环。菜单挂接与首次填充改为幂等操作：标题仅在变化时写入，delegate 已挂接或子菜单已存在时不再重复填充，自身触发的菜单变化通知再次检查时不会继续修改菜单

### 测试

- 全部 290 个测试通过（v2.4.4 为 289 个；新增重复调用 `ensureSubmenu` 不重建菜单项的回归测试）

## [2.4.4] - 2026-09-18

### 修复

- **最近打开菜单在 v2.4.3 中不显示实际记录**：修复 SwiftUI Commands 在无最近项时把子菜单扁平化为单项的问题，改为通过 AppKit 的 `NSMenuDelegate` 在展开时动态填充最近文件/目录，并能在冷启动或外部打开导致的扁平化后自动补建子菜单；同时把侧边栏目录树中的文件切换也纳入最近打开记录，不再只依赖显式打开入口。空状态、最近项、清除菜单与多语言标题均已补齐

### 测试

- 全部 289 个测试通过（v2.4.3 为 285 个；新增菜单动态填充、扁平化自愈、侧边栏文件加载记录、临时 Untitled 文件不记录等回归测试）

## [2.4.3] - 2026-09-18

### 新增

- **更可靠的最近打开记录**：主窗口仅在文件或目录加载成功后写入「打开最近」记录和最后打开位置；打开不支持的文件、缺失文件、缺失目录或加载失败时不再污染最近列表。记录前会标准化文件路径，并校验路径存在及其文件/目录类型，避免同一路径因斜杠、`.`、`..` 等标准化差异重复出现
- **与 macOS 系统最近文档同步**：新增最近项时同步写入 `NSDocumentController` 的系统最近文档，清空应用内最近列表时同步清除系统记录；重复打开同一路径只调整应用内排序，不重复向系统最近文档推送。应用菜单改为通过可观察的共享 `SettingsModel` 驱动，最近项变化后「打开最近」子菜单能及时刷新
- **最近项保留策略**：最近打开项仍按时间倒序排列，同一路径重复打开会移到最前，最多保留 10 条；持久化读写与系统最近文档回调均支持依赖注入，单元测试不再污染真实用户偏好和系统状态

### 测试

- 全部 285 个测试通过（v2.4.2 为 275 个；新增 `RecentItemsTests` 覆盖路径标准化去重、10 条上限、失败不写入、系统最近文档同步/清空、持久化恢复与文件/目录成功打开场景）
