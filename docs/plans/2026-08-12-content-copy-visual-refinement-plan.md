...
**Goal:** 让渲染与编辑模式右上角的内容复制按钮使用同一复制图标、同一紧凑视觉尺寸和同一定位，并移除两者的方形背景、边框和阴影；图标保持小巧，但使用更大的透明触控区以避免难以点击。

**Architecture:** 以 `DetailView` 顶部路径按钮的默认态 `doc.on.doc` 为唯一视觉基准。编辑模式复用该现有 SF Symbol；渲染模式的 DOM SVG 调整为同一“两个重叠文档”的复制图形和相同 14 pt 视觉画布。两个模式都让 14 pt 图标居中于无样式的 24 pt 透明 `Button` 触控区，再以外层坐标维持图标可见位置。按钮仍分别留在 SwiftUI 与 WebKit DOM 中，保留其各自已经验证的复制语义与五秒成功状态。
...
顶部路径复制按钮已经是图标视觉的目标形态：`doc.on.doc`、11 pt 字号、14 pt 宽、无背景/边框/阴影。本任务将两个内容区按钮收敛到这个轻量图标基线，但不把内容区的实际可点击范围缩小为 14 pt。
...
- 两个内容区按钮的**透明触控区**均为 **24 × 24 pt/px**，图标在其中居中；可见图标仍固定在内容视口右上角：距右 16、距上 8。触控区不能出现任何背景、边框、圆角或阴影。
- 为保持上述图标位置，24 pt 触控区的外框坐标为距右 11、距上 3（图标四周各有 5 pt/px 透明余量）；不占正文宽度，不随正文滚动，不改变正文 padding 或最大宽度。
...
**Step 2: Separate visible icon size from the transparent hit target**
...
内层 14 pt frame 只定义图标可见大小；外层 24 pt frame 才是 `Button` 的透明可点击区域。删除图标 label 上的 `.padding(6)`、`.background(...)`、`.overlay(...)` 与 `.shadow(...)`。不要把任何可见容器样式移动到 `Button` 或外层 overlay。

**Step 3: Offset the larger target without moving the visible icon**

将外层按钮定位改为 `.padding(.trailing, 11)` 与 `.padding(.top, 3)`。因为 24 pt 触控区内侧各有 5 pt 透明余量，可见 14 pt 图标仍落在 `trailing: 16 / top: 8`。按钮按模式显隐时只使用 opacity 过渡或不额外过渡；不要再用 scale 使小图标出现时像弹出的方块控件。
...
Expected: build 成功；diff 只涉及内容区编辑复制按钮的 14 pt 图标、24 pt 透明触控区、颜色、装饰移除与非缩放过渡，不触及路径按钮与复制逻辑。
...
保持 flex 居中与 `cursor: pointer`，使其 14 px SVG 在 24 px 的透明点击区域中居中，最终可见图标位置仍为 `top: 8 / right: 16`。`hover` 与 `active` 只能调整 `color`，不得设置 `background`、`border` 或 `box-shadow`。成功 class 只能设置 `color: var(--success)`；去除成功态 border-color。`focus-visible` 允许无填充 outline 来保证键盘可达性。保留 hidden 与 `@media print` 规则。
...
- 可见图标都在内容视口 `top: 8 / right: 16` 处；
- 都显示同一种两张文档复制图标，视觉大小为 14，但透明可点击区域为 24；
- 默认态无任何可见方块、胶囊、边框或阴影；
- hover/active 不出现背景；
- 点击成功后都只显示同尺寸绿色对号五秒，再恢复复制图标。

同时和顶部路径后的复制图标并排目测确认：内容图标不比它更大、更重或更像独立浮动按钮。分别点击图标边缘外约 4–5 pt/px 的透明区域，两个模式都必须可触发复制，确认 24 pt 触控区确实存在而无可见容器。
...
- 渲染与编辑内容区使用同一复制 icon、同一 14 pt 视觉尺寸和同一可见图标右上角坐标；两者均提供 24 pt 透明触控区，标题栏路径按钮保持原样并作为视觉基线。
